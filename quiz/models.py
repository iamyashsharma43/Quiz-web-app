...
class Question(TimeStampedModel):
    ...
    html = models.TextField(gettext('Question Text'))
    is_published = models.BooleanField(gettext('Has been published?'), default=False, null=False)
    maximum_marks = models.DecimalField(gettext('Maximum Marks'), default=1, decimal_places=2, max_digits=5)
...
class Choice(TimeStampedModel):
    ...
    is_correct = models.BooleanField(gettext('Is this correct answer?'), default=False, null=False)
    html = models.TextField(gettext('Choice Text'))
...
class QuizProfile(TimeStampedModel):
    ...
    total_score = models.DecimalField(gettext('Total Score'), default=0, decimal_places=2, max_digits=10)

    def __str__(self):
        return f'<QuizProfile: user={self.user}'

    def get_new_question(self):
        used_questions_pk = AttemptedQuestion.objects.filter(quiz_profile=self).values_list('question__pk',
                                                                                            flat=True)
...
class AttemptedQuestion(TimeStampedModel):
    ...
    is_correct = models.BooleanField(gettext('Was this attempt correct?'), default=False, null=False)
    marks_obtained = models.DecimalField(gettext('Marks Obtained'), default=0, decimal_places=2, max_digits=6)
